...
export const config: ApiRouteConfig = {
  type: 'api',
  name: 'Close Issue',
  description: 'Closes an issue in the configured GitHub repo',
  path: '/close-issue',
  method: 'POST',
...
export const handler: StepHandler<typeof config> = async (input, { logger }) => {
  try {
    ...
    logger.info(`Closed issue #${response.data.number}: ${response.data.title}`);
    return {
      status: 200,
      body: response.data,
    };
  } catch (error: any) {
    logger.error('Error closing issue:', error?.response?.data || error.message || error);
    return {
      status: 500,
